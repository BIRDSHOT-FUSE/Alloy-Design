...
For the later alloy-screening steps, only the composition data was needed. The extracted Nimplex parameters were not required for FLARE or the later property-screening workflow.
...
Separating those parameters makes the composition data easier to handle and reduces the size of the CSV used in the next step.
...
- optional plot file: visual summary of the generated composition space, if available.
...
3. Use the composition-only file in `02_flare_screening/`.
...
- The full raw composition-space output is not the main artifact for this repository.
- This repository focuses on the scripts and the reduced data used in the downstream screening workflow.
